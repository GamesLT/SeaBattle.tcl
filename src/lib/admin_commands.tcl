...
proc ::AdminCommands::handle { nick host handle chan text } {
    ...
    if {[isadmin $nick]<1} {
        ::Say::default "error" $nick $chan [::Language::str "you_are_not_my_admin"];
        return 0;
    }
...
proc ::AdminCommands::doSetHelp {nick chan text} {
    ...
    ::Say::default "error" $nick $chan [::Language::str "help_updated"]
}
...
proc ::AdminCommands::doDetHelp {nick chan text} {
    ...
    set id [::DB::getcell "Help" "id" "item = '$item'"] 
    if { [string trim $id] == "" } {
        ::Say::default "error" $nick $chan [::Language::str "help_cant_remove"]
    } else {
...
proc ::AdminCommands::doSet {nick chan text} {
    ...
    ::Say::default "error" $nick $chan [::Language::str "bot_settings_updated"]
}
...
proc ::AdminCommands::doRehash {nick chan} {
    ::Say::default "error" $nick $chan [::Language::str "rehashing"]
    rehash
}

proc ::AdminCommands::doRestart {nick chan} {
    ::Say::default "error" $nick $chan [::Language::str "restarting"]
    restart
}

proc ::AdminCommands::doDie {nick chan params} {
    ::Say::default "error" $nick $chan [::Language::str "shutdowning"]
    die $params
}
...
proc ::AdminCommands::doChannelsCommand {nick chan text} {
    ...
    switch [string tolower $command] {
        "add" {
            ::Say::default "error" $nick $chan [::Language::str "shutdowning" [list $chan2]]
            channel add $chan2
        }
        "remove" {
            ::Say::default "error" $nick $chan [::Language::str "channel_removed_from" [list $chan2]]
            channel remove $chan2
        }
        "info" {
            set rez [channel info $chan2]
            ::Say::default "error" $nick $chan [::Language::str "channel_info" [list $chan2 $rez]]
        }
    }
}
...
proc ::AdminCommands::doAdmins {nick chan text} {
    ...
    switch [string tolower $command] {
        "add" {
            ...
            ::Say::default "error" $nick $chan [::Language::str "user_got_promoted_to_admin" [list $user]]
        }
        "is" {
            if {[isadmin $user]} {
                ::Say::default "error" $nick $chan [::Language::str "is_admin" [list $user]]
            } else {
                ::Say::default "error" $nick $chan [::Language::str "is_not_admin" [list $user]]
            }
        }
        "remove" {
            set id [::DB::getcell "Users" "id" "nick = '$user'"] 
            if { [string trim $id] == "" } {
                ::Say::default "error" $nick $chan [::Language::str "user_not_found"]
            } else {
                set sql "UPDATE `Users` SET `Admin` = 'false' WHERE `ID` = '$id' LIMIT 1 ;"
                set rez [::DB::exec $sql]
                ::Say::default "error" $nick $chan [::Language::str "admin_revoked" [list $user]]
            }
        }
        "list" {
            ::Say::default "error" $nick $chan [::Language::str "admins_list_start"]
            ...
            while {[set row [::DB:next $query1]]!=""} {
               set i [expr $i+1]
               set is 1
               ::Say::default "error" $nick $chan "$i\.\ $row"
            }
            if {$is==0}    {
                ::Say::default "error" $nick $chan [::Language::str "admins_list_empty"]
            }
